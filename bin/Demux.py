...
#parser.add_argument ("--start-pos", dest="start", type=int, default= 10, help="Start position of barcodes (cero based)")
#parser.add_argument ("--end-pos", dest="end", type=int, default= 46, help="End position of barcodes (cero based)")
#parser.add_argument("-o", "--results_file_name", dest="out_file", default='all_counts.txt',help= "Name of the output file with corrected barcodes")                                
...
print("Count the number of reads in the files")
with dnaio.open(read1) as f:
    bp = 0
    for record in f:
        bp += 1

Qq = int(bp) # reads.shape[0] or smaller number, how many reads to do
