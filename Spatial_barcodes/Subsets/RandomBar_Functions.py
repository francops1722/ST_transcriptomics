...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul  8 16:16:52 2022
...
def make_adjmatrix(edges=[["a", "c", 1], ["c", "g", 1], ["c", "t", 1], ["t", "a", 3],
       ["a", "g", 2], ["c", "t", 2], ["c", "a", 2], ["t", "c", 2],
       ["a", "t", 3], ["c", "a", 1], ["g", "c", 1], ["t", "g", 1],
       ["a", "a", 4], ["c", "c", 4], ["g", "g", 4], ["t", "t", 4]]): 
    #creates a Graph object to calculate number of cycles, input an ar    ray with the edges weights
    G = nx.Graph() #Create graph object
    for i in range(len(edges)):
        G.add_edge(edges[i][0], edges[i][1], weight=edges[i][2])
    return G

def count_cycles(seq, G):
    w = [G.get_edge_data(seq[i],seq[i+1])['weight'] for i in range(0,len(seq)-1)]
    return sum(w)

def chemfilter(seq, G, homomax=3, gmax=7, cyclemax=60) :
    # returns whether seq satisfies chemistry constraints
    bc = bincount(seq,minlength=4)
    if (bc[2] > gmax): return False
    _,_,run_lengths = find_runs(seq)
    if max(run_lengths) > homomax : return False
    if count_cycles(decode(seq), G) > cyclemax: return False
    return True

def chemfilter_2(seq, homomax=3, atmax=22, cgmax=22) :
    # returns whether seq satisfies chemistry constraints
    bc = bincount(seq,minlength=4)
